namespace Microsoft.Dynamics.Obfuscation.Support
{
    ...
    public class USZipCodeObfuscator : IObfuscationAgent
	{
		...
		public bool TryObfuscate(IObfuscationEngine engine, object previous, out object result)
		{
			try
			{
				...
				do
				{
                    ...
                    if (!engine.ContainsReference(string.Format("ZIP={0}/CITY", zip)))
                    {
                        try
                        {
                            ...
                            using (var httpClient = new HttpClient())
                            {
                                ...
                                {
                                    engine.AddOrReplaceReference(string.Format("ZIP={0}/CITY", zip), document.DocumentElement.SelectSingleNode("ZipCode/City").InnerText);
                                    engine.AddOrReplaceReference(string.Format("ZIP={0}/STATE", zip), document.DocumentElement.SelectSingleNode("ZipCode/State").InnerText);
                                }
                                else
                                {
                                    zip = null;
				...
				result = zip;
				engine.AddOrReplaceReference("ZIP", zip);
				return true;
			}
			catch
			{
